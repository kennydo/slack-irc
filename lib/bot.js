...
class Bot {
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      // If the message includes my username but doesn't already have a '@' prefix for
      // my username, send an additional ping.
      if (message.text.search(/kedo/g) != -1 && message.text.search(/@kedo/g) == -1) {
        slackChannel.postMessage({
          parse: 'full',
          username: 'IRC',
          text: '@kedo: You were pinged by ' + author
        })
      }
